...
- Firmware **≥ 1.1.7** flasheado en la placa de banco.
...
| 3 | Esperar `LOW_PUMP_FILL_TIME` | Bomba baja presión (R1) ON |
| 4 | Esperar `PRESSURE_CHECK_TIME` con presostato OK (GPIO33) | Serial: `[EVENT] Presión OK`; `state=PRODUCING`; R2 (alta presión) ON |
...
rearranque puede demorar hasta `RETRY_DELAY` — comportamiento normal de la
FSM (sin cambios), no específico de Chamico.
